...
#[test]
fn real_two_step_online_runtime_closes_recurrent_chain() {
    let result = valid_two_step(false);
...
#[test]
fn first_step_cannot_claim_predecessor_feedback() {
    let first = StepFixture::new(1000, 1000, 9000, false);

    let result = OnlineRecurrentCognitiveLoop::run(
        &a(1000),
        vec![first.input(Some(fb(0, 1000, None)))],
        agent_policy(),
        loop_policy(1),
    );
    ...
    assert_eq!(result.final_anchor_state(), &a(1000));
    assert_eq!(result.rejected_step_index(), Some(0));
}

#[test]
fn second_step_requires_predecessor_feedback() {
    let first = StepFixture::new(1000, 1000, 9000, false);
    let second = StepFixture::new(1003, 2000, 10000, false);

    let result = OnlineRecurrentCognitiveLoop::run(
        &a(1000),
        vec![first.input(None), second.input(None)],
        agent_policy(),
        loop_policy(2),
    );
    ...
    assert_eq!(result.final_anchor_state(), &a(1003));
    assert_eq!(result.rejected_step_index(), Some(1));
}

#[test]
fn feedback_step_index_must_match_exact_predecessor() {
    let first = StepFixture::new(1000, 1000, 9000, false);
    let second = StepFixture::new(1003, 2000, 10000, false);
    ...
    let result = OnlineRecurrentCognitiveLoop::run(
        ...
        vec![
            first.input(None),
            second.input(Some(fb(7, 1003, Some(9200)))),
        ],
        agent_policy(),
        loop_policy(2),
    );
...
#[test]
fn feedback_anchor_must_match_exact_predecessor_anchor() {
    let first = StepFixture::new(1000, 1000, 9000, false);
    let second = StepFixture::new(1003, 2000, 10000, false);
    ...
    let result = OnlineRecurrentCognitiveLoop::run(
        ...
        vec![
            first.input(None),
            second.input(Some(fb(0, 9999, Some(9200)))),
        ],
        agent_policy(),
        loop_policy(2),
    );
...
#[test]
fn feedback_authority_provenance_must_match_exact_predecessor() {
    let first = StepFixture::new(1000, 1000, 9000, false);
    let second = StepFixture::new(1003, 2000, 10000, false);
    ...
    let result = OnlineRecurrentCognitiveLoop::run(
        ...
        vec![
            first.input(None),
            second.input(Some(fb(0, 1003, Some(9999)))),
        ],
        agent_policy(),
        loop_policy(2),
    );
...
#[test]
fn hard_step_frontier_rejects_atomically_before_execution() {
    let first = StepFixture::new(1000, 1000, 9000, false);
    let second = StepFixture::new(1003, 2000, 10000, false);
    ...
    assert_eq!(
        result.status(),
        OnlineRecurrentCognitiveLoopStatus::StepFrontierExceeded
    );
    assert_eq!(result.executed_step_count(), 0);
    assert_eq!(result.completed_step_count(), 0);
    assert_eq!(result.final_anchor_state(), &a(1000));
...
#[test]
fn rejected_online_step_does_not_advance_recurrent_anchor() {
    let first = StepFixture::new(1000, 1000, 9000, false);

    // Structural feedback is valid, but this runtime is deliberately anchored
    // to 9999 rather than the actual predecessor anchor 1003.
    let stale_second = StepFixture::new(9999, 2000, 10000, false);

    let result = OnlineRecurrentCognitiveLoop::run(
        &a(1000),
        vec![
            first.input(None),
            stale_second.input(Some(fb(0, 1003, Some(9200)))),
        ],
        agent_policy(),
        loop_policy(2),
    );
...
#[test]
fn preserve_anchor_feedback_requires_absent_authority_provenance() {
    let first = StepFixture::new(1000, 1000, 9000, true);
    let second = StepFixture::new(1000, 2000, 10000, false);
...
#[test]
fn recurrent_runtime_is_deterministic_and_facade_equivalent() {
    let direct = valid_two_step(false);
    let facade = valid_two_step(true);
    let repeated = valid_two_step(false);
