...
# 1. Új crate Cargo.toml
write_file("crates/athlesia-interactive/Cargo.toml", '''[package]
name = "athlesia-interactive"
version = "0.1.0"
edition = "2021"
...
# 2. src/lib.rs
write_file("crates/athlesia-interactive/src/lib.rs", r'''
use athlesia_types::{Grid, Color};
use rand::Rng; // use rand? We'll avoid external crate by simple RNG
...
# 3. Teszt, amely összehasonlítja az információnyerés-alapú és véletlenszerű felfedezést
write_file("crates/athlesia-interactive/tests/info_gain_benchmark.rs", r'''
...
# 4. Workspace Cargo.toml módosítása új tag hozzáadásával
p = pathlib.Path("Cargo.toml")
s = p.read_text()
old_members = 'members = ["crates/athlesia-types", "crates/athlesia-executor", "crates/athlesia-perception", "crates/athlesia-world-model", "crates/athlesia-features", "crates/athlesia-metalearner", "crates/athlesia-verifier", "crates/athlesia-synthesis", "crates/athlesia-core", "crates/athlesia-search", "crates/athlesia-memory", "crates/athlesia-knowledge", "crates/athlesia-abstraction", "crates/athlesia-hypothesis", "crates/athlesia-planner", "crates/athlesia-kernel"]'
new_members = old_members.replace(']', ', "crates/athlesia-interactive"]')
if old_members in s:
    s = s.replace(old_members, new_members)
else:
    print("[WARNING] A workspace members nem található pontosan; kézzel ellenőrizd.")
    # Fallback: append
    if "crates/athlesia-interactive" not in s:
        s = s.replace(']', ', "crates/athlesia-interactive"]', 1)
p.write_text(s)
print("[5] Workspace Cargo.toml frissítve az új crate-tel.")
...
subprocess.run(["git", "commit", "-m", "Phase 12: interactive information-gain learning with Bayesian hypothesis selection"], check=True)
