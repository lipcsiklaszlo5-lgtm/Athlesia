...
write_file("crates/athlesia-kernel/tests/phase9_external_benchmark.rs", runner_code)
print("[1] phase9_external_benchmark.rs létrehozva.")

# 2. A benchmark futtatása (csak ez a teszt)
result = subprocess.run(
    ["cargo", "test", "-p", "athlesia-kernel", "--test", "phase9_external_benchmark", "--", "--nocapture"],
    capture_output=True,
    text=True,
    check=False,
)
...
if result.returncode != 0:
    print("\n[FAILURE] Phase 9 benchmark nem ment át.")
    sys.exit(1)
...
subprocess.run(["git", "commit", "-m", "Phase 9: external benchmark runner with learning ON/OFF comparison"], check=True)
