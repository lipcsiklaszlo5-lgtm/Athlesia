...
MANIFEST = (
    ROOT
    / "state/module_37_recursive_world_model_revision_discovery_freeze.json"
)

CRATE_MANIFEST = (
    "crates/athlesia_recursive_world_model_revision_discovery/Cargo.toml"
)

FREEZE_STAGE = (
    "module_37_recursive_world_model_revision_discovery_freeze"
)

FREEZE_INVARIANT = (
    "recursive_world_revision_discovery_freeze_integrity"
)


def fail(message):
    print(
        "MODULE 37 RECURSIVE WORLD MODEL "
        "REVISION DISCOVERY VERIFY: FAIL"
    )
...
if FREEZE_STAGE not in completed_layers:
    fail(
        "Module 37 freeze is not recorded "
        "as completed."
    )
...
actual_module_invariants = sorted([
    ...
    if invariant.startswith(
        "recursive_world_revision_discovery_"
    )
...
if clippy.returncode != 0:
    fail(
        "Module 37 clippy failed."
    )

print(
    "MODULE 37 RECURSIVE WORLD MODEL "
    "REVISION DISCOVERY VERIFY: PASS"
)

print(
    "Recursive world revision discovery "
    "integrity gate: "
    + str(len(required))
    + "/"
    + str(len(required))
...
print(
    "Frozen revision discovery files:",
    len(frozen_hashes),
)
