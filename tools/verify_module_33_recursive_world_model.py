...
MANIFEST = (
    ROOT
    / "state/module_33_recursive_world_model_freeze.json"
)

CRATE_MANIFEST = (
    "crates/athlesia_recursive_world_model/Cargo.toml"
)

FREEZE_STAGE = (
    "module_33_recursive_world_model_freeze"
)

FREEZE_INVARIANT = (
    "recursive_world_model_freeze_integrity"
)


def fail(message):
    print(
        "MODULE 33 RECURSIVE WORLD MODEL VERIFY: FAIL"
    )
...
if FREEZE_STAGE not in completed_layers:
    fail(
        "Module 33 freeze is not recorded "
        "as completed."
    )
...
if actual_module_invariants != sorted(required):
    fail(
        "Module 33 invariant set drift detected."
    )
...
if actual_test_count != expected_test_count:
    fail(
        "Recursive world model test count drift: "
...
if tests.returncode != 0:
    fail(
        "Recursive world model tests failed."
    )
...
if clippy.returncode != 0:
    fail(
        "Recursive world model clippy failed."
    )

print(
    "MODULE 33 RECURSIVE WORLD MODEL VERIFY: PASS"
)

print(
    "Recursive world model integrity gate: "
    + str(len(required))
    + "/"
    + str(len(required))
)
...
print(
    "Frozen recursive world model files:",
    len(frozen_hashes),
)
