...
MANIFEST = (
    ROOT
    / "state/module_34_recursive_world_model_evidence_freeze.json"
)

CRATE_MANIFEST = (
    "crates/athlesia_recursive_world_model_evidence/Cargo.toml"
)

FREEZE_STAGE = (
    "module_34_recursive_world_model_evidence_freeze"
)

FREEZE_INVARIANT = (
    "recursive_world_evidence_freeze_integrity"
)


def fail(message):
    print(
        "MODULE 34 RECURSIVE WORLD MODEL "
...
# Progression-safe:
# current project stage may already be Module 35+.
# We only require the Module 34 freeze layer
# to remain recorded in completed_layers.
...
if FREEZE_STAGE not in completed_layers:
    fail(
        "Module 34 freeze is not recorded "
        "as completed."
    )
...
print(
    "Recursive world evidence integrity gate: "
    + str(len(required))
    + "/"
    + str(len(required))
)
...
print(
    "Frozen recursive world evidence files:",
    len(frozen_hashes),
)
...
print(
    "Recursive world evidence tests:",
    actual_test_count,
)
