...
MANIFEST = (
    ROOT
    / "state/module_32_recursive_deliberation_freeze.json"
)

CRATE_MANIFEST = (
    "crates/athlesia_recursive_deliberation/Cargo.toml"
)

FREEZE_STAGE = (
    "module_32_recursive_deliberation_freeze"
)

FREEZE_INVARIANT = (
    "recursive_deliberation_freeze_integrity"
)


def fail(message):
    print(
        "MODULE 32 RECURSIVE DELIBERATION VERIFY: FAIL"
    )
...
if FREEZE_STAGE not in completed_layers:
    fail(
        "Module 32 freeze is not recorded "
        "as completed."
    )
...
if actual_module_invariants != sorted(required):
    fail(
        "Module 32 invariant set drift detected."
    )
...
if actual_test_count != expected_test_count:
    fail(
        "Recursive deliberation test count drift: "
...
if tests.returncode != 0:
    fail(
        "Recursive deliberation tests failed."
    )
...
if clippy.returncode != 0:
    fail(
        "Recursive deliberation clippy failed."
    )

print(
    "MODULE 32 RECURSIVE DELIBERATION VERIFY: PASS"
)

print(
    "Recursive deliberation integrity gate: "
    + str(len(required))
    + "/"
    + str(len(required))
)
...
print(
    "Frozen recursive deliberation files:",
    len(frozen_hashes),
)
